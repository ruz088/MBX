...
///////////////////////////////////////////////////////////////////////////////
//
//  QMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMmmmm


void ElectricFieldHolder::CalcPermanentElecField_QMMM(
    double *xyz1, double *xyz2, double *chg1, double *chg2, size_t mon1_index, size_t mon2_index_start,
    size_t mon2_index_end, size_t nmon1, size_t nmon2, size_t site_i, size_t site_j, size_t ns1, size_t ns2, size_t fi_sites1, size_t fi_sites2,
    double aCC, double aCC1_4, double g34, double *Efqx_mon1, double *Efqy_mon1, double *Efqz_mon1, double *phi1,
    double *phi2, double *Efq2, double elec_scale_factor, double ewald_alpha, bool use_pbc,
    const std::vector<double> &box, const std::vector<double> &box_inverse, double cutoff, bool use_ghost,
    const std::vector<size_t> &islocal, const size_t isl1_offset, const size_t isl2_offset, size_t m2_offset, std::vector<double> *polfac_,
    std::vector<double> *virial) {
    // Shifts that will be useful in the loops
    const size_t nmon12 = nmon1 * 2;
    const size_t nmon22 = nmon2 * 2;
...
#pragma omp simd
    ...
    // Store a*(r/A)^4 in vector
    for (size_t m = mon2_index_start; m < mon2_index_end; m++) {
    double A = (*polfac_)[fi_sites1 + mon1_index*ns1 + site_i] * (*polfac_)[fi_sites2 + m*ns2+site_j];
    double Ai = 0.0;
    double Asqsqi = 0.0;
    if (A > constants::EPS) {
        A = std::pow(A, 1.0 / 6.0);
...
#pragma omp simd
    for (size_t m = mon2_index_start; m < mon2_index_end; m++) {
        ...
        if (accum2) {
            ...
            if (Ai > 1000 ) {
                            const double exp1 = 0;
                            v6_[m] = 0;
               std::cout << " CALLING PSEUDO NO-THOLE " << std::endl;
            }
        ...
        Efq2[site_jnmon23 + nmon2 + m] -= scale * s1r3ci * v1_[m];
            ...
            std::cout << " TS1 TEST " << s1r3 << " V0 " << v0_[m]  << " V1 " << v1_[m] << " V2 " << v2_[m] << std::endl;
            std::cout << " EF TEST " << "s1r3cj v0 " << s1r3cj * v0_[m]  << "s1r3cj v1 " << s1r3cj * v1_[m] << "s1r3cj v2 " << s1r3cj * v2_[m] << std::endl;
            std::cout << " EF TEST " << "s1r3ci v0 " << s1r3ci * v0_[m]  << "s1r3ci v1 " << s1r3ci * v1_[m] << "s1r3cj vi " << s1r3ci * v2_[m] << std::endl;            
            std::cout << " Charges i " << chg1[site_inmon1 + mon1_index] << "  j " << chg2[site_jnmon2 + m] << std::endl;
            std::cout << " EFQ test " <<  Efq2[site_jnmon23 + m] << " " <<  Efq2[site_jnmon23 + nmon2 + m] << " " <<  Efq2[site_jnmon23 + nmon22 + m] << std::endl;
            // update virial
            if (virial != 0) {
    ...
    *Efqz_mon1 = 0.0;
    ...
    for (size_t m = mon2_index_start; m < mon2_index_end; m++) {
        ...
        *Efqy_mon1 += v9_[m];
        *Efqz_mon1 += v10_[m];
        std::cout << "EFQm1 " << *Efqx_mon1 << " " << *Efqy_mon1 << " " << *Efqz_mon1 << std::endl;
        // condensate virial
        if (virial != 0) {
            (*virial)[0] += v11_[0 * maxnmon + m];
            (*virial)[1] += v11_[1 * maxnmon + m];
...
void ElectricFieldHolder::CalcDipoleElecField_QMMM(double *xyz1, double *xyz2, double *mu1, double *mu2, size_t mon1_index,
                                              size_t mon2_index_start, size_t mon2_index_end, size_t nmon1,
                                              size_t nmon2, size_t site_i, size_t site_j, size_t ns1, size_t ns2, size_t fi_sites1, size_t fi_sites2, double aDD,
                                              double *Efd2, double *Efdx_mon1, double *Efdy_mon1, double *Efdz_mon1,
                                              double ewald_alpha, bool use_pbc, const std::vector<double> &box,
                                              const std::vector<double> &box_inverse, double cutoff, bool use_ghost,
                                              const std::vector<size_t> &islocal, const size_t isl1_offset,
                                              const size_t isl2_offset, std::vector<double> *polfac_) {
    // Shifts that will be useful in the loops
    const size_t nmon12 = nmon1 * 2;
    const size_t nmon22 = nmon2 * 2;
    const size_t site_i3 = site_i * 3;
...
void ElectricFieldHolder::CalcElecFieldGrads_QMMM(double *xyz1, double *xyz2, double *chg1, double *chg2, double *mu1,
                                             double *mu2, size_t mon1_index, size_t mon2_index_start,
                                             size_t mon2_index_end, size_t nmon1, size_t nmon2, size_t site_i,
                                             size_t site_j, size_t ns1, size_t ns2, size_t fi_sites1, size_t fi_sites2,  double aDD, double aCD, double *grdx,
                         ...
                         const std::vector<size_t> &islocal, const size_t isl1_offset,
                         const size_t isl2_offset, std::vector<double> *polfac_, std::vector<double> *virial) {
    // Shifts that will be useful in the loops
    const size_t nmon12 = nmon1 * 2;
    const size_t nmon22 = nmon2 * 2;
    const size_t site_i3 = site_i * 3;
