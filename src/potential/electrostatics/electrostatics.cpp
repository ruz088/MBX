...
void Electrostatics::CalculatePermanentElecField_QMMM(bool use_ghost) {
    // Max number of monomers
    std::cout << " QMM PERM FIELD " << std::endl;
    size_t maxnmon = mon_type_count_.back().second;
    ElectricFieldHolder elec_field(maxnmon);
...
#pragma omp parallel  // omp_get_num_threads() needs to be inside
    ...
    for (size_t mt = 0; mt < mon_type_count_.size(); mt++) {
        ...
        for (size_t i = 0; i < ns - 1; i++) {
            ...
            for (size_t j = i + 1; j < ns; j++) {
                ...
                size_t mstart = (mpi_rank_ < nmon) ? mpi_rank_ : nmon;
                for (size_t m = mstart; m < nmon; m += num_mpi_ranks_) {
                double A = polfac_[fi_sites +m*ns+ i] * polfac_[fi_sites +m*ns+ j];
                double Ai = 0.0;
                double Asqsqi = 0.0;
                if (A > constants::EPS) {
                    A = std::pow(A, 1.0 / 6.0);
                    ...
                    elec_field.CalcPermanentElecField_QMMM(
                        xyz_.data() + fi_crd, xyz_.data() + fi_crd, chg_.data() + fi_sites, chg_.data() + fi_sites, m,
                        m, m + 1, nmon, nmon, i, j,
                        ns, ns, fi_sites,fi_sites,
                       // Ai, Asqsqi,
                    ...
                    Efq_[fi_crd + inmon3 + nmon + m] += ey;
                    Efq_[fi_crd + inmon3 + nmon2 + m] += ez;
                    std::cout << " EFQ ELEC " << Efq_[fi_crd + inmon3 + m] << " "  <<  Efq_[fi_crd + inmon3 + nmon + m] << " " <<  Efq_[fi_crd + inmon3 + nmon2 + m]
                    << std::endl;
                }
            }
        }
    ...
    std::cout << " FINISHED INTRA MONOMER SHIT " << std::endl;
#ifdef _DEBUG_PERM
    {  // debug print
        int me, nprocs;
        MPI_Comm_size(world_, &nprocs);
...
#pragma omp parallel for schedule(dynamic)
            ...
            for (size_t m1 = m1start; m1 < nmon1; m1 += num_mpi_ranks_) {
                ...
                for (size_t i = 0; i < ns1; i++) {
                    ...
                    for (size_t j = 0; j < ns2; j++) {
                        ...
                        double elec_scale_factor = 1;
                        local_field->CalcPermanentElecField_QMMM(
                            xyz_.data() + fi_crd1, xyz_sitej.data(), chg_.data() + fi_sites1, chg_sitej.data(), m1, 0,
                            size_j, nmon1, size_j, i, 0,ns1, ns2, fi_sites1,fi_sites2, aCC_, aCC1_4_, g34_, &ex_thread, &ey_thread,
                            &ez_thread, &phi1_thread, phi_sitej.data(), Efq_sitej.data(), elec_scale_factor,
                            ewald_alpha_, use_pbc_, box_, box_inverse_, cutoff_, use_ghost, islocal_, fi_mon1 + m1,
                            fi_mon2, m2init, &virial_thread);
                        ...
                        Efq_1_pool[rank][inmon13 + nmon1 + m1] += ey_thread;
                        Efq_1_pool[rank][inmon13 + nmon12 + m1] += ez_thread;
                        std::cout << " Ez_thread " << ez_thread << " Efq1_pool " << Efq_1_pool[rank][inmon13 + nmon12 + m1] << std::endl;
                        // update virial_pool from virial_threads
                        for (size_t k = 0; k < 9; k++) {
                            virial_pool[rank][k] += virial_thread[k];
                        }
    ...
    for (size_t i=0; i< Efq_.size(); i++) {
        ...
        std::cout << " EFQ " << i << " " << Efq_[i] << std::endl;
    }
...
void Electrostatics::CalculateDipoles() {
    ...
    if (dip_method_ == "iter")
        ...
        CalculateDipolesAspc();
    else if (dip_method_ == "qmmm")
        CalculateDipolesCG_QMMM();
...
void Electrostatics::DipolesCGIteration_QMMM(std::vector<double> &in_v, std::vector<double> &out_v) {
    // Apply sqrt(pol) to the dipoles
    int fi_sites = 0;
    int fi_crd = 0;
    int fi_mon = 0;
    for (size_t mt = 0; mt < mon_type_count_.size(); mt++) {
        ...
        for (size_t i = 0; i < ns; i++) {
            ...
            //double A = -std::sqrt(pol_[fi_sites + i]);
            for (size_t m = 0; m < nmon; m++) {
                double A = -std::sqrt(pol_[fi_sites + m*ns + i]);
                in_v[fi_crd + inmon3 + m] *= A;
                in_v[fi_crd + inmon3 + nmon + m] *= A;
                in_v[fi_crd + inmon3 + nmon2 + m] *= A;
            }
...
void Electrostatics::CalculateDipolesCG_QMMM() {
    // Parallelization
    //    size_t nthreads = 1;
    //#   ifdef _OPENMP
    //#     pragma omp parallel // omp_get_num_threads() needs to be inside
...
#pragma omp simd
    ...
    DipolesCGIteration_QMMM(mu_, ts2v);
    ...
    while (true) {
        ...
        std::cout << "Iteration: " << iter << std::endl;
#endif
        DipolesCGIteration_QMMM(pv, ts2v);
        double pvts2pv = DotProduct(pv, ts2v);
...
void Electrostatics::ComputeDipoleField_QMMM(std::vector<double> &in_v, std::vector<double> &out_v, bool use_ghost) {
    // Parallelization
    size_t nthreads = 1;
#ifdef _OPENMP
#pragma omp parallel  // omp_get_num_threads() needs to be inside
        ...
        for (size_t i = 0; i < ns - 1; i++) {
            ...
            for (size_t j = i + 1; j < ns; j++) {
                ...
                for (size_t m = mstart; m < nmon; m += num_mpi_ranks_) {
                    ...
                    // TODO. Slowest function
                    elec_field.CalcDipoleElecField(xyz_.data() + fi_crd, xyz_.data() + fi_crd, in_ptr + fi_crd, // use the old one because on same monomer
                                                   in_ptr + fi_crd, m, m, m + 1, nmon, nmon, i, j, Asqsqi, aDD, // and we can define the two sites outside the loop
                                                   out_v.data() + fi_crd, &ex, &ey, &ez, ewald_alpha_, use_pbc_, box_,
                                                   box_inverse_, cutoff_, use_ghost, islocal_, fi_mon + m, fi_mon);
                    out_v[fi_crd + inmon3 + m] += ex;
                    out_v[fi_crd + inmon3 + nmon + m] += ey;
...
#pragma omp parallel for schedule(dynamic)
            ...
            for (size_t m1 = m1start; m1 < nmon1; m1 += num_mpi_ranks_) {
                ...
                for (size_t i = 0; i < ns1; i++) {
                    size_t inmon13 = 3 * nmon1 * i;
                    for (size_t j = 0; j < ns2; j++) {
                        double A = polfac_[fi_sites1 + m1*ns1+ i] * polfac_[fi_sites2 + ns2 + j];
                        double Ai = 0.0;
                        double Asqsqi = 0.0;
                        if (A > constants::EPS) {
                            A = std::pow(A, 1.0 / 6.0);
                        ...
                        local_field->CalcDipoleElecField_QMMM(xyz_.data() + fi_crd1, xyz_.data() + fi_crd2, in_ptr + fi_crd1,
                                                         in_ptr + fi_crd2, m1, m2init, nmon2, nmon1, nmon2, i, j, ns1, ns2, fi_sites1, fi_sites2,
                                                         aDD, Efd_2_pool[rank].data(), &ex_thread, &ey_thread,
                                                         &ez_thread, ewald_alpha_, use_pbc_, box_, box_inverse_,
                                                         cutoff_, use_ghost, islocal_, fi_mon1 + m1, fi_mon2, &polfac_);
                        Efd_1_pool[rank][inmon13 + m1] += ex_thread;
                        Efd_1_pool[rank][inmon13 + nmon1 + m1] += ey_thread;
                        Efd_1_pool[rank][inmon13 + nmon12 + m1] += ez_thread;
                    }
...
double Electrostatics::GetElectrostatics_QMMM(std::vector<double> &grad, std::vector<double> *virial, bool use_ghost) {
    std::fill(virial_.begin(), virial_.end(), 0.0);
    CalculatePermanentElecField_QMMM(use_ghost);
    CalculateDipoles();
    CalculateElecEnergy();
    if (do_grads_) CalculateGradients(grad);
    // update viral
    ...
    has_energy_ = true;
    std::cout << " E IND INTERNAL is " << Eind_ << std::endl;
    return Eperm_ + Eind_;
}
